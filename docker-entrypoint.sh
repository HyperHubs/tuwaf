...
import re
import os
out=re.sub('(VirtualHost\s\*:8001)','VirtualHost *:'+os.environ['PORT'],open('/etc/apache2/conf-enabled/proxy.conf','r').read())
open('/etc/apache2/conf-enabled/proxy.conf','w').write(out)
EOF
) && \
...
import re
import os
out=re.sub('(Listen\s+8001)','Listen            '+os.environ['PORT'],open('/etc/apache2/conf-enabled/proxy.conf','r').read())
open('/etc/apache2/conf-enabled/proxy.conf','w').write(out)
EOF
) && \
...
import re
import os
out=re.sub('(http://localhost:8002)',os.environ['BACKEND'],open('/etc/apache2/conf-enabled/proxy.conf','r').read())
open('/etc/apache2/conf-enabled/proxy.conf','w').write(out)
EOF
) && \
